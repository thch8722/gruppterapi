...
<?php /* How to display all other posts. */ ?>
	...
	<?php else : ?>
		<div id="post-<?php the_ID(); ?>" class="<?php echo (++$j % 2 == 0) ? 'evenpost' : 'oddpost'; ?> post">
			<h2 class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Länk till %s', 'skeleton' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
	...
	<?php else : ?>
			<div class="entry-content">
				<?php the_content( __( 'Läs vidare <span class="meta-nav">&rarr;</span>', 'skeleton' ) ); ?>
				<?php wp_link_pages( array( 'Före' => '<div class="page-link">' . __( 'Pages:', 'skeleton' ), 'Efter' => '</div>' ) ); ?>
			</div><!-- .entry-content -->
	<?php endif; ?>
			...
			<div class="entry-utility">
				<?php if ( count( get_the_category() ) ) : ?>
					<span class="cat-links">
						<?php printf( __( '<span class="%1$s">Postat i kategori</span> %2$s', 'skeleton' ), 'entry-utility-prep entry-utility-prep-cat-links', get_the_category_list( ', ' ) ); ?>
					</span>
					<span class="meta-sep">|</span>
				<?php endif; ?>
				<?php
				...
				?>
					<span class="tag-links">
						<?php printf( __( '<span class="%1$s">Etiketter</span> %2$s', 'skeleton' ), 'entry-utility-prep entry-utility-prep-tag-links', $tags_list ); ?>
					</span>
					<span class="meta-sep">|</span>
				<?php endif; ?>
				<span class="comments-link"><?php comments_popup_link( __( 'Lämna en kommentar', 'skeleton' ), __( '1 Comment', 'skeleton' ), __( '% Comments', 'skeleton' ) ); ?></span>
				<?php edit_post_link( __( 'Edit', 'skeleton' ), '<span class="meta-sep">|</span> <span class="edit-link">', '</span>' ); ?>
			</div><!-- .entry-utility -->
		</div><!-- #post-## -->
...
<?php if (  $wp_query->max_num_pages > 1 ) : ?>
				<div id="nav-below" class="navigation">
					<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Äldre inlägg', 'skeleton' ) ); ?></div>
					<div class="nav-next"><?php previous_posts_link( __( 'Nyare inlägg <span class="meta-nav">&rarr;</span>', 'skeleton' ) ); ?></div>
				</div><!-- #nav-below -->
<?php endif; ?>
